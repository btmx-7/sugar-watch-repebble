...
- [x] 0.9 `pebble build` — verify both fonts load without warnings (SDK not installed; pending emulator test)
...
- [x] 5.4 Verify on emulator: no clipping outside circular boundary (gabbro emulator verified)
- [x] 5.5 Adjust if needed — no adjustment required
...
- [x] 9.1 `pebble build` — zero warnings, zero errors, all 5 platforms (emery, gabbro, basalt, diorite, chalk)
- [x] 9.2 `pebble install --emulator emery` — Simple layout, all 4 slots assigned, arc strokes visible
- [x] 9.3 Screenshot T2 Simple: 2-row time prominent, 4 corner slots, arc strokes, BT icon
- [x] 9.4 `pebble install --emulator emery` — switch to Dashboard layout, verify graph + top slots
- [x] 9.5 Screenshot T2 Dashboard: time center, 3 top slots, sparkline graph, trend panel
